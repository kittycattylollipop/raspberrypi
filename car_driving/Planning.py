...
class MotionPlanning:
    def __init__(self):
        ...
        # parameters
        self.param_car_timeout = 0.5 # 0.8 # 0.5  # second
        self.param_car_timeout_forward_large = 0.7
        self.param_car_timeout_turn_small = 0.1 # 0.3  #0.2
        self.param_car_timeout_turn_large = 0.25  # 0.7 (too fast) #0.4

        self.param_forward_speed = 800
        self.param_turn_speed = 1600
        self.param_turn_speed_inner_wheel = 1400
    ...
    def step(self, arena_floor, time_out_factor=1):
        try:
    ...
    def avoid_collision(self, arena_floor):
        if arena_floor.dist_to_obj[0] < self.param_collision_dist_th:    # distance in cm
            if self.motor_on:
                ...
                self.car.turn_right(self.param_turn_speed, self.param_turn_speed_inner_wheel)
                # sleep for long turn
    ...
    def barrel_finding_closest_first(self, arena_floor):
        if len(arena_floor.red_barrels_in_view) == 0 and len(arena_floor.green_barrels_in_view) == 0:
            ...
            self.car_timeout = self.param_car_timeout_turn_large
            if self.motor_on:
                self.car.turn_right(self.param_turn_speed, self.param_turn_speed_inner_wheel)
            ...
            if barrels_loc[barrel_ind] > 0 and max_width <= self.param_far_object_width:
                if self.motor_on:
                    ...
                    self.car_timeout = self.param_car_timeout_forward_large
                    self.car.move_forward(self.param_forward_speed, self.param_forward_speed)
            elif abs(closest_barrel[5] / arena_floor.img_width - 0.5) < self.param_img_ctr_offset:
                if self.motor_on:
                    print("MotionPlanning-barrel_finding: barrel close to center, move closer")
                    self.car_timeout = self.param_car_timeout
                    self.car.move_forward(self.param_forward_speed, self.param_forward_speed)
            else:
                # turn to move the barrel to the center
                # reduce the timer
                self.car_timeout = self.param_car_timeout_turn_small
                if closest_barrel[5] > arena_floor.img_width / 2:
                    ...
                    if self.motor_on:
                        self.car.turn_left(self.param_turn_speed_inner_wheel, self.param_turn_speed)
    ...
    def barrel_catching(self, arena_floor):
        ...
        if arena_floor.barrel_in_arm:  # barrel is already in arm
            ...
            if len(arena_floor.center_barrel) > 0:  # barrel at center, but not in arm yet
                ...
                speed_adjust_ratio = min(1.0, np.abs(dist_err) / self.param_img_ctr_offset) * self.param_speed_adjust
                ...
                if dist_err > 0:  # the target is to the right of the center
                    ...
                    print("MotionPlanning-barrel_catching: move toward barrel, slight right, adjust_speed =  % s"
                          % adjust_speed)
                    if self.motor_on:
                    ...
                    print("MotionPlanning-barrel_catching: move toward barrel, slight left, adjust_speed = % s"
                          % adjust_speed)
                    if self.motor_on:
